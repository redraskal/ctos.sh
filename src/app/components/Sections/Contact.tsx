    ...
    <section>
      <div className="border-l-2 border-white/50 pl-4 py-2">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-3 sm:gap-y-2 gap-x-2 mb-4 text-base">
          <ContactLink label="EMAIL" value={contactInfo.email} href={`mailto:${contactInfo.email}`} external={false} />

          <ContactLink label="GITHUB" value={contactInfo.github} href={`https://github.com/${contactInfo.github}`} />
          ...
          <ContactLink label="PUBLIC KEYS" value={contactInfo.publicKeys} href={`https://${contactInfo.publicKeys}`} />
        </div>
      </div>
    </section>
  );
